...
function ChatPage() {
    ...
    useEffect(() => { if (!loading && !user)
        navigate({ to: "/" }); }, [user, loading, navigate]);
    useEffect(() => {
    ...
    return (<div className="h-screen flex bg-background overflow-hidden">
      <div className={`${mobileShowChat ? "hidden" : "flex"} md:flex w-full md:w-auto`}>
        <ContactsPanel selectedChat={selected} online={online} onSelectChat={(id, p, name, avatar) => {
            setSelected(id);
            setPeer(p ?? null);
            setHeaderName(name ?? p?.name ?? null);
            setHeaderAvatar(avatar ?? p?.avatar_url ?? null);
            setMobileShowChat(true);
        }} onCreateGroup={() => setGroupOpen(true)}/>
      </div>
      <div className={`${mobileShowChat ? "flex" : "hidden"} md:flex flex-1`}>
        <ChatPanel chatId={selected} peer={peer} headerName={headerName} headerAvatar={headerAvatar} online={online} onBack={() => setMobileShowChat(false)}/>
      </div>
